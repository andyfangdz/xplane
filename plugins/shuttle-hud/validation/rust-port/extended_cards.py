...
with XPlaneApi(port=8144,timeout=8) as a:
 a.refresh_catalogs();init(a,'verified-143-css-digital-base',height=20,along=300,pitch=11,gamma=-.5,eas=240,gear=1)
 results={}
 try:
  for mode in [0,1,2,3]:
   a.set_dataref('fsim_hud/declutter_mode',mode);time.sleep(.2)
   s=snap(a,f'verified-143-air-declutter-{mode}');assert s['fsim_hud/declutter_level']==mode
  ...
  s=snap(a,'verified-143-auto-final-flare');assert s['fsim_hud/control_auto']==1 and s['fsim_hud/display_flags']&4;results['auto']=s
  a.set_dataref('sim/cockpit/autopilot/autopilot_mode',0);a.command('fsim_hud/att_ref',.2);time.sleep(.2)
  s=snap(a,'verified-143-att-ref-caged');assert s['fsim_hud/att_ref_caged']==1;results['cage']=s
  a.command('fsim_hud/att_ref',.2)
  a.set_dataref('sim/operation/override/override_planepath',[1]+[0]*19);advance(a,5.2)
  s=snap(a,'verified-143-gear-down-expired');assert s['fsim_hud/gear_cue']==0;results['gear_timer']=s
  ...
  for name,target in [('on',.12),('off',.68)]:
   ...
   while time.monotonic()<end:
    ...
    time.sleep(.025)
   s=snap(a,'verified-143-gear-warning-'+name);assert s['fsim_hud/gear_cue']==1
  ...
  (R/'extended-143.json').write_text(json.dumps(results,indent=2))
