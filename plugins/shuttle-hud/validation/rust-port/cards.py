...
def snap(a,name,kind='initialized display card'):
 ...
 return data['state']
def init(a,name,height=8500,along=-9500,pitch=-10,bank=0,heading=HEAD,gamma=-20,eas=300,gear=0,aircraft_path='Aircraft/Testing/Shuttle Rust Trial/Orbiter_Glider.acf'):
 config('disable')
 try:
...
if __name__=='__main__':
 with XPlaneApi(port=8144,timeout=8) as a:
  a.refresh_catalogs()
  if sys.argv[1]=='first':init(a,'verified-143-first',height=7000,along=-8150)
   ...
   for name,values in specs:init(a,'verified-143-'+name,**values)
