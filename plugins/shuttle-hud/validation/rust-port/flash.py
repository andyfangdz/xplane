...
with XPlaneApi(port=8144,timeout=8) as a:
 ...
 try:
  init(a,'trial-143-alert-base',height=150,along=-700,pitch=20,gamma=-1.5,eas=250,gear=0,aircraft_path='Aircraft/Testing/Shuttle Rust Trial/Orbiter_Glider.acf')
  ...
  for k in range(6):
   advance(a,.27)
   s=snap(a,f'trial-143-flash-{k}','initialized alert card; actual simulation time advanced between paused captures')
  ...
  (R/'release-flash-143.json').write_text(json.dumps(out,indent=2))
